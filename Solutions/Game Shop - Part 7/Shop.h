...
public:
	...
	void ShowSellMessage(Player& player) const;

	void BuyItem(Player& player, size_t index, int numItems) const;
	void SellItem(Player& player, size_t index, int numItems) const;

	std::shared_ptr<Item> FindItemByName(std::string itemName);
